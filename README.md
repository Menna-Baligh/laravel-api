...
---
# API Documentation Setup with Scalar & Scribe

This project uses **Scalar** and **Scribe** to generate and serve API documentation.
...
2. Publish Scalar configuration and views:
   ```bash
   php artisan vendor:publish --tag="scalar-config"
   php artisan vendor:publish --tag="scalar-views"
   ```

3. Install Scribe package:
   ```bash
   composer require knuckleswtf/scribe
   ```

4. Publish Scribe configuration:
   ```bash
   php artisan vendor:publish --tag=scribe-config
   ```

5. Generate OpenAPI documentation using Scribe:
   ```bash
   php artisan scribe:generate
   ```

6. Move the generated `openapi.yaml` file to the `public` directory.

7. Update Scalar configuration (`config/scalar.php`) and set the URL to:
   ```php
   'url' => '/openapi.yaml',
   ```
...
## Result
   ...
   ```
Now you can access your API documentation using Scalar, and it will serve the OpenAPI specification generated by Scribe.
